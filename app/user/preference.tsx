...
export default function PreferencePage() {
  ...
  const [selected, setSelected] = useState<number[][]>(PREFERENCE_DATA.map(() => []));

  // 체크박스 토글
  const toggleCheck = (catIdx: number, optIdx: number) => {
    setSelected((prev) =>
      prev.map((arr, i) =>
        i === catIdx ? (arr.includes(optIdx) ? arr.filter((idx) => idx !== optIdx) : [...arr, optIdx]) : arr,
      ),
    );
  };
  ...
  return (
    <ScrollView style={styles.scrollContainer} keyboardShouldPersistTaps="handled">
      ...
      <ScrollView style={{ width: '100%', maxHeight: 500 }} contentContainerStyle={{ paddingBottom: 24 }}>
          ...
          <View key={cat.label} style={styles.formLabel}>
            ...
            >
              {cat.options.map((opt, optIdx) => (
                ...
                >
                  <Checkbox checked={selected[catIdx].includes(optIdx)} onPress={() => toggleCheck(catIdx, optIdx)} />
